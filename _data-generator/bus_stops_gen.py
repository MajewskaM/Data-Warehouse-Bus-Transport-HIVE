...
bus_stops_file_path = os.path.join(data_folder, "Bus_Stops_2020.txt")
    ...
    for region, cities in region_postalcode_city.items():
        for city in cities.values():
            ...
            if airport:
                ...
                row = f"{bus_stop_id},{bus_stop_name}{bus_stop_counter} - Airport,{longitude},{latitude},{sitting_place},{airport},{bus_shelter}"
            ...
            airport = 0
            bus_shelter = np.random.randint(2)
            row = f"{bus_stop_id},{bus_stop_name}{bus_stop_counter},{longitude},{latitude},{sitting_place},{airport},{bus_shelter}"
            Bus_Stops.append(row)
            bus_stop_id += 1
...
def get_bus_stop_names(city_name):
    city_stops_ids = [stop.split(',')[1]
